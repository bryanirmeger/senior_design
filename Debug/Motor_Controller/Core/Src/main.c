...
uint8_t transmit_start[6] = {'s','t','a','r','t','\n'};
...
void motors_controller(float speed) {
	poll_motor = 0;
	poll_speed(&htim1);
	error = speed - w_time[0];
	while(error > error_t) {
		poll_motor = 0;
		poll_speed(&htim1);
		error = speed - w_time[0];
		//calc v from requested error. calc dc from error v. calc ccr from dc
		v1 = ((error / 60) / 10) * period;
		//change ccr
		__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_2, (uint16_t) v1);
